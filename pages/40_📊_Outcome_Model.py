...
st.set_page_config(page_title="Outcome Model", page_icon=":bar_chart:",layout="wide")
...
if 'variable_col' not in st.session_state:
    ...
    if st.session_state.variable_col != st.session_state.target_outcome_col:  
        if st.session_state.is_vcol_numeric:
            ...
            with c3:
                ...
                data1 = df[df[st.session_state.target_outcome_col] == st.session_state.target_outcome_value][st.session_state.variable_col]
                data2 = df[df[st.session_state.target_outcome_col] != st.session_state.target_outcome_value][st.session_state.variable_col]
                ...
                if (data1.empty or data2.empty):
                    ...
                    try:
                        ...
                        fig.update_layout(width=800, height=600, xaxis_title=st.session_state.variable_col, yaxis_title="Density Within Group",  legend_title= st.session_state.variable_col + ' value when target outcome')
                        ...
                        fig.add_scatter(x=x1, y=y1,fill='tozeroy', mode='none' , fillcolor='rgba(191,239,241,0.5)', opacity=0.2, name = st.session_state.target_outcome_value)

                        config = {  'displayModeBar': True,
                                    'toImageButtonOptions': {
                                        'filename': 'outcome_model',
                                        'height': 1024,
                                        'width': 1280
                                    }
                                }
        ...
        st.error('The specific variable is same as the target outcome')